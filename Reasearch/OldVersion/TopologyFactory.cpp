...
{
	double xAggregator = positions.at(0).first;
	double yAggregator = positions.at(0).second;
	...
	for(int i=0; i!= m_corrTopology.size(); ++i)
	{
		for(int j=0; j!=m_corrTopology[i].size(); ++j)
		{
			...
			{
				double x0 = positions.at(i+1).first;
				double y0 = positions.at(i+1).second;
				double x1 = positions.at(j+1).first;
				double y1 = positions.at(j+1).second;
				double capacityToAggregator = CapacityCal(x1,y1,xAggregator,yAggregator,m_power,m_bandwidth,m_N0); // j to aggregator
				double timeToTxIdt = m_corrTopology.at(j).at(j) / capacityToAggregator; //transmission time for camera j
				double linkCapacity = CapacityCal(x0,y0,x1,y1,m_power,m_bandwidth,m_N0);
				if(timeToTxIdt*linkCapacity < m_corrTopology.at(j).at(j))
					m_corrTopology.at(i).at(j) = -1; // -1 means fail to overhear
			}
		}	
	}
...
void TopologyFactory::PrintTopology()
{
	cout << "Required transmission bytes: (-1 means cannot overhear)" << endl;    
	for (int i=0; i!=m_numCameras; ++i)
    {
        for (int j=0; j!=m_numCameras; ++j)
            cout << m_corrTopology.at(i).at(j) << " ";
        cout << endl;
    }
}
