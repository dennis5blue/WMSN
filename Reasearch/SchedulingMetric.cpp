...
using namespace std;
SchedulingMetric::SchedulingMetric(int numCameras, vector< pair<double,double> > positions, vector< vector<int> > overhearTopology):
...
vector< vector<double> > SchedulingMetric::CalRequiredTime()
{
	power = 0.125; // Watt
...
double SchedulingMetric::CapacityToAggregatorCal(double m_x, double m_y)
{
	double dx = m_x-xA;
	double dy = m_y-yA;
...
double SchedulingMetric::ResourceDifferenceCal(int cam1, int cam2)
{
...
double SchedulingMetric::MetricCal(vector<int> m_nonScheduled, int m_targetCam)
{
	double m_metric = 0.0;
	double omega = 0.0;
...
int SchedulingMetric::ScheduledCameraPosition(vector<int> m_nonScheduled, int m_eraseCam)
{
	int pos = 0;
	for (int i=0; i!=m_nonScheduled.size(); ++i)
	{
...
void SchedulingMetric::PrintSchedule()
{
...
void SchedulingMetric::PrintIntVector(vector<int> &targetVector)
{
	for (int i=0; i!=targetVector.size(); ++i)
		cout << targetVector.at(i) << " ";
	cout << endl;
}
void SchedulingMetric::PrintDoubleVector(vector<double> &targetVector)
{
	for (int i=0; i!=targetVector.size(); ++i)
		cout << targetVector.at(i) << " ";
	cout << endl;
