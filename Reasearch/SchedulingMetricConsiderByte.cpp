...
using namespace std;
SchedulingMetricConsiderByte::SchedulingMetricConsiderByte(int numCameras, vector< pair<double,double> > positions, vector< vector<int> > overhearTopology):
...
vector<int> SchedulingMetricConsiderByte::GenerateSchedule()
{
...
vector< vector<double> > SchedulingMetricConsiderByte::CalRequiredTime()
{
	power = 0.125; // Watt
...
double SchedulingMetricConsiderByte::CapacityToAggregatorCal(double m_x, double m_y)
{
	double dx = m_x-xA;
	double dy = m_y-yA;
...
int SchedulingMetricConsiderByte::ResourceDifferenceCal(int cam1, int cam2)
{
	int difference = 0;
...
int SchedulingMetricConsiderByte::MetricCal(vector<int> m_nonScheduled, int m_targetCam)
{
	int m_metric = 0;
	int omega = 0;
...
int SchedulingMetricConsiderByte::ScheduledCameraPosition(vector<int> m_nonScheduled, int m_eraseCam)
{
	int pos = 0;
	for (int i=0; i!=m_nonScheduled.size(); ++i)
	{
...
void SchedulingMetricConsiderByte::PrintSchedule()
{
...
void SchedulingMetricConsiderByte::PrintTopology()
{
...
void SchedulingMetricConsiderByte::PrintIntVector(vector<int> &targetVector)
{
	for (int i=0; i!=targetVector.size(); ++i)
		cout << targetVector.at(i) << " ";
	cout << endl;
}
void SchedulingMetricConsiderByte::PrintDoubleVector(vector<double> &targetVector)
{
	for (int i=0; i!=targetVector.size(); ++i)
		cout << targetVector.at(i) << " ";
	cout << endl;
